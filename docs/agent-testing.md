...
```bash
curl -s -X POST http://127.0.0.1:8010/agent -H "Content-Type: application/json" \
  -d '{"message":"帮我算 (125+75)*0.08"}'
```
...
```bash
curl -s -X POST http://127.0.0.1:8010/documents/upload -F "file=@员工手册.md" \
...
```bash
curl -s -N -X POST http://127.0.0.1:8010/agent/stream -H "Content-Type: application/json" \
  -d '{"message":"帮我算 2+3"}'
```
...
| 步骤 | 操作 | 预期 |
|------|------|------|
| 1 | 勾选「🤖 Agent 模式（自动调用工具）」 | 聊天切换为 agent 模式 |
...
python -m pytest tests/test_api.py -q          # 1. 24 个全绿
curl -s http://127.0.0.1:8010/health           # 2. UP
# 3. 算数 → calculator；4. 时间 → get_current_time；5. 手册问题 → search_knowledge_base
# 6. use_mcp:true（未配置时）→ 503；7. max_iterations:11 → 422
```
